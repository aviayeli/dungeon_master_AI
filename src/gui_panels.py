...
class LeftPanel(tk.Frame):
    ...
    def _build(self) -> None:
        ...
        self._name_var = tk.StringVar(value=rtl("שם: ---"))
        self._class_var = tk.StringVar(value=rtl("מקצוע: ---"))
        self._loc_var = tk.StringVar(value=rtl("מיקום: ---"))
        for var in (self._name_var, self._class_var, self._loc_var):
    ...
    def refresh(self, stats: dict) -> None:
        self._name_var.set(rtl(f"שם: {stats.get('name', '---')}"))
        self._class_var.set(rtl(f"מקצוע: {stats.get('character_class', '---')}"))
        self._loc_var.set(rtl(f"מיקום: {stats.get('location', '---')}"))
        ...
        self._inv.insert("end", "\n".join(rtl(f"• {i}") for i in items) if items else rtl("ריק"))
...
class RightPanel(tk.Frame):
    ...
    def refresh(self, stats: dict, cost: dict) -> None:
        ...
        self._tokens_var.set(rtl(
            f"קלט: {cost.get('input_tokens', 0)} | פלט: {cost.get('output_tokens', 0)}"
        ))
