...
class LeftPanel(tk.Frame):
    ...
    def _build(self) -> None:
        tk.Label(self, text="🛡 שחקן", bg=PANEL_BG, fg=ACCENT,
        ...
        for label, var in [("שם:", self._name_var), ("מקצוע:", self._class_var), ("מיקום:", self._loc_var)]:
        ...
        tk.Label(self, text="חיים:", bg=PANEL_BG, fg=ACCENT,
        ...
        tk.Label(self, text="מלאי:", bg=PANEL_BG, fg=ACCENT,
                 font=("Arial", 10, "bold"), anchor="e").pack(fill="x", padx=10, pady=(10, 2))
    ...
    def refresh(self, stats: dict) -> None:
        ...
        self._inv.insert("end", "\n".join(f"• {i}" for i in items) if items else "ריק")
...
class RightPanel(tk.Frame):
    ...
    def _build(self) -> None:
        tk.Label(self, text="📊 סטטיסטיקה", bg=PANEL_BG, fg=ACCENT,
                 font=("Arial", 13, "bold"), anchor="e").pack(fill="x", padx=10, pady=(10, 6))

        self._turns_var = tk.StringVar(value="תורות: 0")
        self._cost_var = tk.StringVar(value="עלות: $0.000000")
        self._tokens_var = tk.StringVar(value="קלט: 0 | פלט: 0")
        ...
        tk.Label(self, text="יומן אירועים:", bg=PANEL_BG, fg=ACCENT,
    ...
    def refresh(self, stats: dict, cost: dict) -> None:
        self._turns_var.set(f"תורות: {stats.get('turn_count', 0)}")
        usd = cost.get("total_cost_usd", 0.0)
        self._cost_var.set(f"עלות: ${usd:.6f}")
        self._tokens_var.set(f"קלט: {cost.get('input_tokens', 0)} | פלט: {cost.get('output_tokens', 0)}")
