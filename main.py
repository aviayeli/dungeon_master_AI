...
def _new_game_dialog(root: tk.Tk) -> tuple[str, str] | None:
    ...
    dialog.title("משחק חדש")
    dialog.configure(bg=BG)
    dialog.resizable(False, False)
    dialog.grab_set()

    tk.Label(dialog, text="ברוך הבא, גיבור!", bg=BG, fg=ACCENT,
             font=("Arial", 16, "bold")).pack(pady=(20, 4))
    tk.Label(dialog, text="מה שמך?", bg=BG, fg=TEXT,
             font=("Arial", 11)).pack()
    ...
    def _confirm() -> None:
        ...
        if not n:
            messagebox.showwarning("שגיאה", "יש להזין שם!", parent=dialog)
            return
        result.append((n, class_var.get()))
        dialog.destroy()

    tk.Button(dialog, text="התחל הרפתקה! ⚔", bg=ACCENT, fg="white",
...
def _start_session(root: tk.Tk, sdk: DungeonMasterSDK) -> bool:
    ...
    if save_found:
        if messagebox.askyesno("ברוך הבא", "נמצא משחק שמור. האם לטעון אותו?", parent=root):
            return True
